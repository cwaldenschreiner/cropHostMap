...
google_root = "Q:"
google_dir_path = f"{google_root}/My Drive"
project_dir = f"{google_root}/Shared drives/APHIS  Projects/eRADS"
...
# Query Cropland Data Layer for identified year(s) and host(s)
for year in rast_years:
    print(
        f"{year}: Acquiring Cropland Data Layer & checking classification accuracies..."
    )
    ...
    cdl_accuracy_df = pd.read_csv(
        f"{project_dir}/data/cdl_accuracy_eval/{year}/cdl_accu_filtered_wType.csv"
    ).set_index("Category")
    ...
    for host in host_list:
        ...
        if (cdl_accuracy_df.loc[host, "Producer"] >= threshold) and (
            cdl_accuracy_df.loc[host, "User"] >= threshold
        ):
            ...
            files_to_move = glob.glob(
                f"{google_dir_path}/{export_folder_name}/*{host_name}_{year}*"
            )
            ...
            for uid in unique_ids:
                ...
                my_ras = gdal.Warp(
                    ...
                    options=gdal.WarpOptions(
                        dstSRS="EPSG:4326", creationOptions=GDAL_CO
                    ),
                )
                my_ras = None
                print("\t\t\tDONE")
                ...
                print(
                    cdl_accuracy_df.loc[host, "Producer"],
                    (cdl_accuracy_df.loc[host, "User"]),
                )
...
for host in host_list:
    ...
    reclass_stack_dst_path = (
        stack_dst_dir + f"/{host}/{host}_stack_{start_year}-{end_year}_reclassified.tif"
    )
    ...
    print("\tsaving...")
    write_raster(reclass_stack_dst_path, da.nansum(stack, axis=0), **profile)
